...
## 子语料补充：multihop-news（英文新闻多跳问答）

`knowledge/multihop-news/` 是英文新闻语料，专门用于多跳推理评估。
当问题涉及人名/公司/产品/事件且明显是英文世界的新闻话题时，按以下流程处理。

### 检索策略

1. **第一步：跳过 data_structure.md 的层级导航**（这个目录是扁平的，索引文件只是文章列表）
2. **直接 grep 关键实体**：
   - 提取问题中的所有专有名词（人名、公司名、产品名、地名）
   - 用 `grep -l -i "实体名" knowledge/multihop-news/` 找命中的文件
   - 优先 grep **罕见词**（如 "Bankman-Fried" 比 "trial" 命中更精准）
3. **多跳问题的标准动作**：
   - **comparison（对比）**：分别 grep 每个对比对象，各拿到 1-2 篇相关文章
   - **temporal（时序）**：grep 相同事件，再按文章内 `published_at` 元数据排序
   - **inference（推理）**：先找最罕见的实体，再以该实体为锚点 grep 相关概念
   - **null（无答案）**：尝试 3 种关键词组合都查不到 → 填 `not_found`
4. **每篇命中文件的读取方式**：
   - 用 `read_file(path, offset=0, limit=120)` 读前 120 行（包含 Metadata + Body 开头）
   - 如果开头不够回答，继续 `offset=100, limit=80` 读后续段落
   - **每个文件不要超过 200 行的读取量**

### 自评估规则（multihop-news 专用）

填 `status` 时要求：

- **success**：找到 ≥ ground-truth 暗示的文章数（一般 2-4 篇），且证据片段直接回答问题
- **partial**：找到一半左右文章，或找到正确文章但片段不完整
- **not_found**：grep 多个关键词都返回 0 命中 → 主题不在语料内
- **uncertain**：grep 命中很多但无法判断哪个最相关 → 触发兜底召回

### evidences JSON 字段（multihop-news 专用）

每条 evidence 必须包含：

- `source_path`: `knowledge/multihop-news/{filename}.md`
- `source_type`: `"md"`
- `locator`: 段落标题或行号区间，如 `"Body / 段落 3"` 或 `"line 45-80"`
- `snippet`: 直接从文章中复制粘贴的相关原文（≤ 300 字符）
- `score`: 0~1，越高越相关
- `parent_id`: 留空字符串

**重要**：snippet 必须是从文章原文摘出来的事实陈述，不要总结、不要改写。
